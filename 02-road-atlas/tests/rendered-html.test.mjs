...
test("server-renders the finished trip experience", async () => {
  ...
  assert.match(html, /THE ROAD ATLAS/i);
  assert.match(html, /Day[\s\S]{0,30}7[\s\S]{0,30}of[\s\S]{0,30}17/i);
  assert.match(html, /Big Sur Coast Highway/i);
  assert.match(html, /Open in Google Maps/i);
