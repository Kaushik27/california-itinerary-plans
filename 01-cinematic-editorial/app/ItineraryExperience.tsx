...
function RouteMap({ activeDay, onSelect }: { activeDay: number; onSelect: (day: number) => void }) {
  ...
  useEffect(() => {
    ...
    void import("leaflet").then((L) => {
      ...
      L.polyline(points, { color: "#17345b", weight: 9, opacity: 0.13, lineCap: "round" }).addTo(map);
      L.polyline(points, { color: "#287fea", weight: 4, opacity: 0.68, lineCap: "round" }).addTo(map);
      progressLine.current = L.polyline([points[0]], { color: "#dc8a22", weight: 5, opacity: 1, lineCap: "round" }).addTo(map);
...
export default function ItineraryExperience() {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      if (savedChecks) setReadyChecks(JSON.parse(savedChecks));
    });
    ...
    <div className="site-shell">
      ...
      <header className="topbar">
        <a className="brand" href="#top" aria-label="California trip home">
          <span><b>CALIFORNIA / 26</b><small>Anniversary road trip</small></span>
        </a>
        <nav aria-label="Trip sections">
          <a href="#route-story">Route</a>
          <a href="#itinerary">17 days</a>
          <a href="#stays">Stays</a>
          <a href="#budget">Budget</a>
          <a href="#book">Flights & SUV</a>
          <a href="#checks">Trip checks</a>
        </nav>
      ...
      <main id="main-content">
        <section className="hero" id="top">
          ...
          <div className="hero-copy">
            ...
            <p className="hero-date"><CalendarDays size={18} /> {tripData.meta.dateRange} · {tripData.meta.travelers} travelers</p>
            <div className="hero-actions">
              <a className="button primary" href="#route-story">Explore the route <ChevronRight size={18} /></a>
              <a className="button secondary" href="#itinerary"><CalendarDays size={18} /> Open all 17 days</a>
            </div>
            <div className="countdown" aria-label={`${countdown.days} days and ${countdown.hours} hours until departure`}>
              <span><b>{countdown.days}</b> days</span><i /> <span><b>{countdown.hours}</b> hours</span><small>until takeoff</small>
            </div>
          </div>
        </section>

        <section className="journey-strip" aria-label="Trip sequence and statistics">
          <div className="journey-route">
            <span className="journey-script">The journey</span>
            <ol>
              <li><b>Chicago</b><small>ORD</small><ChevronRight className="journey-arrow" size={15} /></li>
              <li><b>San Francisco</b><small>SFO</small><ChevronRight className="journey-arrow" size={15} /></li>
              <li><b>Yosemite</b><small>NP</small><ChevronRight className="journey-arrow" size={15} /></li>
              <li><b>Pacific Coast</b><small>CA-1</small><ChevronRight className="journey-arrow" size={15} /></li>
              <li><b>Los Angeles</b><small>LAX</small><ChevronRight className="journey-arrow" size={15} /></li>
              <li><b>Las Vegas</b><small>LAS</small><ChevronRight className="journey-arrow" size={15} /></li>
              <li><b>Chicago</b><small>ORD</small></li>
            </ol>
          </div>
          <div className="journey-stats"><span>17 days</span><i /> <span>16 nights</span><i /> <span>1,529 mapped miles</span><i /> <span>10 bases</span></div>
        </section>
              ...
              <div className="story-content">
                <div className="story-toolbar">
                  ...
                  <button className="story-play" type="button" onClick={() => setStoryPlaying(!storyPlaying)} aria-pressed={storyPlaying}>
                    {storyPlaying ? <Pause size={16} /> : <Play size={16} />} {storyPlaying ? "Pause" : "Play"}
                  </button>
                </div>
                <p className="story-date">{formatDate(active.date)} · Sleep in {active.base}</p>
                <h3>{active.city}</h3>
                <p className="story-theme">{cleanCopy(active.theme)}</p>
                ...
                <div className="story-actions">
                  ...
                  <button type="button" onClick={() => selectDay(active.day, true)}>See day {active.day} <ChevronRight size={18} /></button>
              ...
              return (
                <article className={`day-card tone-${day.tone} ${isExpanded ? "is-expanded" : ""}`} id={`day-${day.day}`} key={day.day}>
                  <button className="day-summary" type="button" onClick={() => { setExpandedDay(isExpanded ? 0 : day.day); setActiveDay(day.day); }} aria-expanded={isExpanded} aria-controls={`day-body-${day.day}`}>
                    ...
                    <span className="day-title"><small>{formatDate(day.date)} · {toneLabels[day.tone]}</small><b>{day.city}</b><em>{cleanCopy(day.theme)}</em></span>
